...
- [ ] **Frontend Integration:** Scaffold `wwwroot/app.js`. Wire up event listeners to capture the image and prompt, construct the `FormData`, and make the asynchronous `fetch` call to `/api/vision/analyze`. Integrate professional loading indicators and gracefully handle error states.
